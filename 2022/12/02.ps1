...
{
    ## reset the map
    foreach($n in $map){$n.cost = [int]::MaxValue; $n.visited = $false}
    $startNode.cost = 0  
    ...
    return $map[$endLocation[0],$endLocation[1]].Cost
}

$startNode = $map[$startLocation[0],$startLocation[1]]
$minSteps = [int]::MaxValue
$i = 0
foreach($startNode in $map.Where({$_.Height -eq 0}))
{
    $i++
    Write-Output "Testing Path $i"
    $steps = Dijkstra $startNode
    if($steps -lt $minSteps){$minSteps = $steps}
}
$minSteps