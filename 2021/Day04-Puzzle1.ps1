...
for($i = 0; $i -lt $bingoBoardList.Count; $i += 6)
{
    ...
    $bingoBoards += [pscustomobject]@{Board = $boardArray; CalledNumbers = @()}
}

:caller foreach($number in $calledNumbers)
{
...
$uncalledNumbers = $winningBoard.Board | Where-Object {$_ -notin $winningBoard.CalledNumbers}
$sum = 0
