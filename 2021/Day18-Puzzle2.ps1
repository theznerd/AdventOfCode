...
{
    $reducedPuzzleInput = $puzzleInput.Where({$_ -ne $number})
    foreach($secondNumber in $reducedPuzzleInput)
    {
        $numberToReduce = Invoke-AddNumber $number $secondNumber
        $reducedNumber = Invoke-ReduceNumber $numberToReduce
        $magnitudeOfNumber = [int]::Parse((Get-NumberMagnitude $reducedNumber))
        if($magnitudeOfNumber -gt $maxMagnitude)
        {
            $maxMagnitude = $magnitudeOfNumber
        }
    }
}
$maxMagnitude
